...
const OnboardingProvider = ({ children }: Props) => {
  ...
  const initOnboarding = async (): Promise<void> => {
    ...
    if (path == '/login/' || path == '/login' || path == '/logout/' || path == `/register/` || path == `/register/${window.location.search}` || path == `/confirm-password/${window.location.search}` || path == `/confirm-user/${window.location.search}` || path == '/forgot-password/' || path == '/xerosignup/' || path == '/xerosignup' || router.asPath === `/thankyou`|| path == `/thankyou/${window.location.search}`) {
      return;
    }
